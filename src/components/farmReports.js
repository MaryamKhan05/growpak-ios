...
const FarmReports = () => {
  ...
  const filterDataHandler = (pressed) => {
    ...
    if (
      pressed.ulist_label === "پانی کی دستیابی کا تجزیہ" ||
      pressed.ulist_label === "پودوں کی صحت کا تجزیہ"
    ) {
      let a = f.filter(
        (item) =>
          item.ulist_label === "پودوں کی صحت اور پانی کی دستیابی کا تجزیہ"
      );
      console.log(a, "پودوں کی صحت اور پانی کی دستیابی کا تجزیہ");
      setFilteredData(a);
    } else {
      let a = f.filter((item) => item.ulist_label === pressed.ulist_label);
  ...
  const categoriesRenderItem = ({ item }) => {
    return (
      ...
      >
        <Text style={styles.categoryCardText}>{item.ulist_label}</Text>
      </TouchableOpacity>
    );
  };
