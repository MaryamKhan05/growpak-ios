...
import { FarmReports, Forecast } from "../../components/index";
import {
...
const SubscribedFarm = () => {
  ...
  const [nitrogenImagery, setNitrogenImagery] = useState(null);
  const [carbonImagery, setCarbonImagery] = useState(null);
  const [waterImagery, setWaterImagery] = useState(null);
  const [healthImagery, setHealthImagery] = useState(null);
  const [cropName, setCropName] = useState("Wheat");
  const [serviceType, setServiceType] = useState();
  const [carbonButton, setCarbonButton] = useState(true);
  const [nitrogenButton, setNitrogenButton] = useState(true);
  const [waterButton, setWaterButton] = useState(true);
  ...
  //     setHealthImagery(newArray);
  //   } else {
  //     setHealthImagery(null);
  //   }
  // }, [plantHealthData]);
  ...
  //     setWaterImagery(newArray);
  //   } else {
  //     setWaterImagery(null);
  //   }
  // }, [waterData]);
  ...
  //     setCarbonImagery(newArray);
  //   } else {
  //     setCarbonImagery(null);
  //   }
  // }, [carbonData]);

  // directly using the nitrogenData variable in the flatlist
  // useEffect(() => {
  //   if (nitrogenData) {
  ...
  //       };
  //     });
  //     console.log(newArray, "newarray new new nwew new enw w");
  //     setNitrogenImagery(newArray);
  //   } else {
  //     setNitrogenImagery(null);
  //   }
  // }, [nitrogenData]);
  ...
  const renderItem = ({ item }) => {
    console.log(item, "water water water ");
    return (
  ...
  const renderItemForLegends = ({ item }) => {
    return (
      <View style={{ flexDirection: "row" }}>
        <Text style={styles.qualityRowText}>{item.title}</Text>
        <View
          style={{
      ...
      {farm && nitrogen ? (
        <View style={[styles.imagery, { top: isPressed ? hp(47) : hp(68) }]}>
          ...
          >
            ...
            <Text style={styles.imageryText}>ٹائم سیریز</Text>
          </View>
          {isPressed ? (
            <FlatList
              horizontal
        ...
        <TouchableWithoutFeedback onPress={() => setSubModal(false)}>
          <View style={styles.dotCenteredView}>
            <View style={styles.dotModalView}>
              ...
              >
                <Text style={styles.modalText}>یہ فارم سبسکرائب نہیں ہے۔ </Text>
                <TouchableOpacity
                  onPress={() => [
